...
@Model.register("bigram_embedder_dimwise_ws")
class BigramEmbedderDimwiseWeightedSum(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        super(BigramEmbedderDimwiseWeightedSum, self).__init__(vocab, regularizer)
    ...
    def forward(self,  # type: ignore
        ...
        dim = word1_vector.size()[1]
        sum_weights = self.mapping_layer(torch.cat([word1_vector, word2_vector], dim=-1))

        sum_weights_1, sum_weights_2 = sum_weights[:, :dim], sum_weights[:, dim:]

        bigram_vecs_hat = sum_weights_1 * word1_vector + sum_weights_2 * word2_vector
